...
export async function GET(req: Request) {
  try {
    ...
    if (ordersError) {
      console.error('[print/packing] Failed to fetch orders:', ordersError);
      return NextResponse.json({ ok: false, error: "Failed to fetch orders" }, { status: 500 });
    }

    // Fetch all stops for this ring, ordered by order_index
    ...
    if (stopsError) {
      console.error('[print/packing] Failed to fetch stops:', stopsError);
      return NextResponse.json({ ok: false, error: "Failed to fetch stops" }, { status: 500 });
    }

    // Group orders by stop
    ...
    console.error('[print/packing] Exception:', error);
